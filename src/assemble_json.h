...
extern AJsonValue	*assemble_json_parse_single_pass(char *string, uint string_length, int write_protected, AJsonType number_type); /* parses a string of json and creates a tree structure of AJasonValues. If write_protected, the Parsing and the resulting tree will be significantly faster but not modifyable. "number_type" needs to be a A_JT_NUMBER_FLOAT, a A_JT_NUMBER_DOUBLE or a A_JT_NUMBER_DECIMAL, and will specify how parsed numbers are stored internaly. All numberd values can be read read/written using number get/set function disregarding of how it is stored internally. This version of the parser forgoes any validation and will crash if the json string is in any way broken. The implementation will allocate lagest possible memory footprint, estimated from the "string_length" parameter, and willtherfor we very memory wastefull. */
...
extern void			assemble_json_value_number_get_decimal(uint8 *value, AJsonDecimalNumber *decimal); /* Fills out a AJsonDecimalNumber of the value if of type A_JT_NUMBER, A_JT_NUMBER_FLOAT, A_JT_NUMBER_DOUBLE, or A_JT_NUMBER_DECIMAL. */
extern uint			assemble_json_value_number_set_decimal(uint8 *value, AJsonDecimalNumber *decimal);/* Sets the AJsonDecimalNumber of the value to "decimal", if of type A_JT_NUMBER, A_JT_NUMBER_FLOAT, A_JT_NUMBER_DOUBLE, or A_JT_NUMBER_DECIMAL. */
